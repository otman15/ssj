...
public class DigitalNetBase2 extends DigitalNet {
    ...
    */
   private int randomBitVectorInt(RandomStream stream, int numBits) {
      if (numBits < 1)
   ...
   public void nestedUniformScramble(RandomStream stream, double[][] output, int numBits) {
      ...
      assert outDigits <= 31;  // Maximum allowed in this version.
      assert numBits <= 30;    // Maximum allowed in this version.

      if (numBits == 0)
         numBits = outDigits-1;
      // System.out.println("NUS: outDigits = " + outDigits);
      double localNormFactor = 1.0 / Math.abs((double) (1L << (outDigits)));
      System.out.println("NUS: outDigits = " + outDigits);
      System.out.println("normFactor = " + localNormFactor);
      ...
      for (int j = 0; j < dim; ++j) {
         ...
         // System.out.println("bvlist[numPoints-1] = " + bvlist[numPoints-1]);
         for (int b = 0; b < 4; b++) {
            for (int i = 0; i < 256; i++)
               counts[i] = 0;
            int m = (b % 2) * numPoints;
         ...
         //System.out.println("bvlist[0] = " + Integer.toBinaryString(bvlist[0]));
         //System.out.println("bvlist[numPoints-1] = " + bvlist[numPoints-1]);
         // Note that there nothing random until here.  
         // The previous only initializes a data structure and it could be done only once
         // for any given `k` and `genMat`.  It can be recomputed only when `genMatNUS` changes.  
         int bv = randomBitVectorInt(stream, numBits);   // Same bv for all points, but different bv2.
         output[poslist[0]][j] = (bvlist[0] ^ bv) * localNormFactor + EpsilonHalf; 
         //System.out.println("bv = " + Integer.toBinaryString(bv) + ",  log_2 bv = " + Math.log(bv)/Math.log(2.0));
         //System.out.println("output[poslist[0]][j] = " + output[poslist[0]][j]);
         //System.out.println("i = " + 0 + ", bvlist[0] = " + Long.toBinaryString(bvlist[0]));
         for (int i = 1; i < numPoints; i++) {
            // System.out.println("i = " + i + ", bvlist[i] = " + Long.toBinaryString(bvlist[i]));
            int bv2 = bvlist[i - 1];
            bv2 ^= bvlist[i];
            bv2 = randomBitVectorInt(stream, numBits) & ((int) (1 << (int) Num.log2((double) bv2)) - 1);
            bv ^= bv2;
            // Here, the value of `poslist[i]` differs for the different coordinates j.
            // This makes it much more difficult to avoid storing the points. 
            output[poslist[i]][j] = (bvlist[i] ^ bv) * localNormFactor + EpsilonHalf;
            // System.out.println("bv2 = " + Integer.toBinaryString(bv2));
         }
      }
   }
   ...
   public void nestedUniformScramble64(RandomStream stream, double[][] output, int numBits) {
      ...
      // System.out.println("NUS: outDigits = " + outDigits);
      // System.out.println("normFactor = " + localNormFactor);
      // System.out.println("EpsilonHalf = " + EpsilonHalf);
      int[] poslist = new int[2 * numPoints];
      ...
      for (int j = 0; j < dim; ++j) {
         ...
         for (int b = 0; b < 4; b++) {
            ...
            long bv = 0xff << bb;
            for (int i = 0; i < numPoints; i++)
               counts[(int)((bvlist[m + i] & bv)) >>> bb]++;   // unsigned right shift
            binpos[0] = (1 - b % 2) * numPoints;
            for (int i = 0; i < 255; i++)
               binpos[i + 1] = binpos[i] + counts[i];
            for (int i = 0; i < numPoints; i++) {
         ...
         // System.out.println("bvlist[0] = " + bvlist[0]);
         // System.out.println("bvlist[numPoints-1] = " + bvlist[numPoints-1]);
         
         // Note that there nothing random until here.  
         // The previous only initializes a data structure and it could be done only once
         // for any given `k` and `genMat`.  It can be recomputed only when `genMatNUS` changes.  
         // Only the two arrays `bvlist` and `poslist` needs to be retained from this initialization. 
         // `bvlist` should be saved in `long`, since it is the way it is used.
         //
         // long bv = randomBitVectorLong(stream, numBits);   // Same bv for all points, but different bv2.
         long bv = (stream.nextLong(0, (1L << numBits)-1) << (outDigits - numBits));
         long bvlistL = bvlist[0] << (outDigits - 31);
         output[poslist[0]][j] = (bvlistL ^ bv) * localNormFactor + EpsilonHalf; 
         // System.out.println("numBits = " + numBits);
         // System.out.println("bv = " + bv + ",  log_2 bv = " + Math.log(bv)/Math.log(2.0));
         // System.out.println("output[poslist[0]][j] = " + output[poslist[0]][j]);
         //System.out.println("i = " + 0 + ", bvlist[0] = " + Long.toBinaryString(bvlist[0]));
         for (int i = 1; i < numPoints; i++) {
            // System.out.println("i = " + i + ", bvlist[i] = " + Long.toBinaryString(bvlist[i]));
            long bv2 = (bvlist[i - 1]) << (outDigits - 31);
            bvlistL = (bvlist[i]) << (outDigits - 31);
            bv2 ^= bvlistL;
   ...
   public void setupNUS64(int numBits) {
      ...
      if (numBits == 0)
         numBitsNUS = outDigits;
      else
         numBitsNUS = numBits;
      normFactorNUS = 1.0 / Math.abs((double) (1L << (outDigits)));
      // streamNUS = stream;
      // System.out.println("NUS: outDigits = " + outDigits);
      // System.out.println("normFactor = " + localNormFactor);
      poslistNUS = new int[2 * numPoints][dim];
      poslistInvNUS = new int[numPoints][dim];
      bvlistNUS = new long[2 * numPoints][dim];
      int[] counts = new int[256];
      int[] binpos = new int[256];

      // In case we memorize them, there must be one structure for each dimension j.
      // The structure depends on j via the generating matrices,
      for (int j = 0; j < dim; ++j) {
         bvlistNUS[0][j] = 0;
         poslistNUS[0][j] = 0;
         for (int i = 1; i < numPoints; i++) {
            ...
            // This depends on j.
            bvlistNUS[i][j] = bvlistNUS[i - 1][j] ^ (genMat[j * numCols + pos] >>> (outDigits - 31));
            poslistNUS[i][j] = i;
         }
         // System.out.println("bvlist[numPoints-1] = " + bvlist[numPoints-1]);
         for (int b = 0; b < 4; b++) {
            ...
            long bv = 0xff << bb;
            for (int i = 0; i < numPoints; i++)
               counts[(int)((bvlistNUS[m + i][j] & bv)) >>> bb]++;   // unsigned right shift
            binpos[0] = (1 - b % 2) * numPoints;
            for (int i = 0; i < 255; i++)
               binpos[i + 1] = binpos[i] + counts[i];
            for (int i = 0; i < numPoints; i++) {
               int pos = (int)((bvlistNUS[m + i][j] & bv) >>> bb);
               int k = binpos[pos]++;
               bvlistNUS[k][j] = bvlistNUS[m + i][j];
               poslistNUS[k][j] = poslistNUS[m + i][j];
            }
         }
         for (int i = 0; i < numPoints; i++) 
   ...
   */
   ...
   public void nestedUniformScramble(RandomStream stream, double[][] output, int numBits) {
      ...
      long[][] long_output = new long[numPoints][dim];
      nestedUniformScramble(stream, long_output, numBits);
      for (int j = 0; j < dim; ++j) {
         for (int i = 0; i < numPoints; i++) {
            output[i][j] = long_output[i][j] * normFactor + EpsilonHalf;
         }
      }
   }
*/
   /**
    * Same as @link nestedUniformScramble(RandomStream,double[][],int)@endlink, but
    * it returns the points as integers between 0 and 2^outDigits - 1, instead of doubles.
    * 
    * NOTE: We can scramble at most 31 bits of output. ***
    */
   
/**   public void nestedUniformScramble(RandomStream stream, long[][] output, int numBits) {
      assert output.length == numPoints;
      assert output.length > 0;
      assert output[0].length == dim;
      ...
      System.out.println("NUS: numPoints = " + numPoints + ", outDigits = " + outDigits);
      for (int j = 0; j < dim; ++j) {
         bvlist[0] = 0;
         poslist[0] = 0;
         for (int i = 1; i < numPoints; i++) {
            ...
            bvlist[i] = bvlist[i - 1] ^ (int)(genMat[j * numCols + pos] >> (outDigits - numRows)); 
            poslist[i] = i;
         }
         for (int b = 0; b < 4; b++) {
            for (int i = 0; i < 256; i++)
            ...
            int bv = 0xff << bb;
            for (int i = 0; i < numPoints; i++)
               counts[bvlist[m + i] & bv >>> bb]++;   // bvlist must be int.
            binpos[0] = (1 - b % 2) * numPoints;
            for (int i = 0; i < 255; i++)
               binpos[i + 1] = binpos[i] + counts[i];
            for (int i = 0; i < numPoints; i++) {
               int pos = (bvlist[m + i] & bv) >>> bb;
                System.out.println("NUS: pos = " + pos + ", bvlist[m+i] = " + bvlist[m+i]);
               int k = binpos[pos]++;
               System.out.println("NUS: k = " + k + ", m = " + m + ", i = " + i);
               bvlist[k] = bvlist[m + i];
               poslist[k] = poslist[m + i];
            }
         }
         int bv = randomBitVectorInt(stream, numBits);
         output[poslist[0]][j] = ((long)(bvlist[0] ^ bv)) << (outDigits - numRows);
         for (int i = 1; i < numPoints; i++) {
            int bv2 = bvlist[i - 1];
            bv2 ^= bvlist[i];
            bv2 = randomBitVectorInt(stream, numBits) & ((int) (1 << (int) Num.log2((double) bv2)) - 1);
            bv ^= bv2;
            output[poslist[i]][j] = ((long)(bvlist[i] ^ bv)) << (outDigits - numRows);
         }
      }
   }
*/
